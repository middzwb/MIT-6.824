...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
    ...
    rf.mu.Lock()
    defer rf.mu.Unlock()
    if rf.curr_term > args.Term { // why not curr_term >= args.Term
...
func Make(peers []*labrpc.ClientEnd, me int,
    ...
    rf.update_timeout()
    //dprintf("make peers size=%v", len(peers))
    go rf.timeout_check()
...
func (rf *Raft) handle_send_request_vote(term int, s int) {
    ...
    for ok := rf.sendRequestVote(s, args, reply); !ok ; {
        rf.mu.Lock()
        if rf.curr_term > args.Term {
            rf.mu.Unlock()
            return
...
func (rf *Raft) to_candidate() {
    ...
    for i, _ := range rf.peers {
        if i != rf.me {
            go rf.handle_send_request_vote(rf.curr_term, i)
        }
    }
}
...
func (rf *Raft) heartbeat_entry() {
    ...
    for !rf.killed() {
        for rf.is_leader() {
            ...
            for i, _ := range rf.peers {
                if i != rf.me {
                    go rf.handle_append_entries(tmp, i)
                }
            }
...
func (rf *Raft) handle_append_entries(t int, s int) {
    args := &AppendEntriesArgs{
        Term: t,
        LeaderId: rf.me,
    }
    reply := &AppendEntriesReply{}
    for ok := rf.peers[s].Call("Raft.AppendEntries", args, reply); !ok ; {
        rf.mu.Lock()
        if rf.curr_term > args.Term {
            rf.mu.Unlock()
            return
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
    ...
    if rf.curr_term > args.Term {
        ...
        reply.Term = rf.curr_term
        return
    } else if rf.curr_term < args.Term {
        rf.to_follower(args.Term)
    } else {
        reply.Success = true
    }
